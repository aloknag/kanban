...
### Epic #7 AC8 — print stylesheet hid every column header and task card, not just chrome controls

**Bug ID:** Epic #7 AC8 gap
**Why introduced:** The print stylesheet's "hide interactive UI chrome" rule targeted `[role="button"]` on the assumption that role would only appear on small controls (the Done-column collapse toggle). But `@dnd-kit`'s `useSortable()`/`useDraggable()` spread a full `role="button"` (plus `tabIndex`, `aria-roledescription`, etc.) onto the entire draggable surface for keyboard-drag accessibility — the whole `<header>` in `SortableColumn.tsx` and the whole task-card wrapper `<div>` in `SortableTaskCard.tsx`, not just the actual `<button>` element inside the header. `[role="button"]` therefore matched and `display: none !important`'d every column header (title + specimen count) and every task card on the board in print, leaving only the real collapse buttons correctly hidden and nothing else visible.
**Who/where:** `frontend/src/styles/index.css` — the `@media print` "Hide interactive UI chrome" rule (originally added per FrontEngDesign.md §13 #5/§17 #1) listed `[role="button"]` alongside the bare `button` tag selector, which was already sufficient on its own to hide the real `<button>` collapse control.
**Where it should have been caught:** A test that actually renders the dnd-kit-wrapped components and checks the real print CSS selectors against them — `frontend/src/styles/print.test.tsx` existed but was tautological (it created detached elements, set an attribute, then asserted the element had the attribute it was just given; never parsed or matched against the real stylesheet). `SortableColumn.test.tsx` even had a comment noting "the header which is also a button via DnD" right next to the collapse-button test, but nothing connected that fact to the print rule.
**What to avoid:** When a print/visibility rule targets an ARIA role or other attribute set by a third-party library (dnd-kit, Radix, etc.) rather than a semantic HTML tag, verify what that library actually attaches the attribute to — accessibility roles are often spread onto much larger interactive regions than the "control" a selector author has in mind. Prefer hiding via an explicit `[data-print-hide]` marker on the specific element to hide, not a role selector that a third-party library also uses elsewhere. And a print-CSS test that never parses the real stylesheet, and never renders the real component tree it's meant to guard, cannot catch a rule being too broad — it can only confirm the test's own fixtures agree with themselves.
